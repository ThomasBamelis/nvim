return {
	-- Puts the command box in the middle of screen and shows warning in popup boxes
	{
		...
		keys = {
			...
			{ "<leader>nf", function() if not require("noice.lsp").scroll(4) then return "<c-f>" end end, silent = true, expr = true, desc = "Scroll Forward (c-f)", mode = {"i", "n", "s"} },
			{ "<c-f>", function() if not require("noice.lsp").scroll(4) then return "<c-f>" end end, silent = true, expr = true, desc = "Scroll Forward", mode = {"i", "n", "s"} },
			{ "<leader>nb", function() if not require("noice.lsp").scroll(-4) then return "<c-b>" end end, silent = true, expr = true, desc = "Scroll Backward (c-b)", mode = {"i", "n", "s"}},
